...
//    esp_err_t esp_intr_alloc(int source, int flags, intr_handler_t handler, void *arg, intr_handle_t *ret_handle)
    ...
    esp_err_t errRc = esp_intr_alloc(
    ...
    while (1) {
        ...
        printf("The Interrupt Cound is %u\n", interruptCound );
        setup_triangle_sine_waves(test_bits);
        vTaskDelay(5000/portTICK_RATE_MS);

        printf("nWaiting 5 more seconds...\n");
        vTaskDelay(5000/portTICK_RATE_MS);
        // using 24 or 32 bits is not making any signal on the DAC #2 pin GPIO 26 
    /*    test_bits += 8;
        if(test_bits > 32)
            test_bits = 16; */
